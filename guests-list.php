<?php
    ...
    echo '<h1 class="list-header">Guests</h1>';
    echo '<table>';
    echo '<thead>';
    echo '<tr>';
    echo '<td>ID</td>';
    ...
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        $guestID = $row['guestID'];
        echo '<tr><td>' . $row['guestID'] . '</td>' . '<td>' . $row['guestFirstName'] . '</td>' . '<td>' . $row['guestLastName'] . '</td>' . '<td>' . $row['guestEmail'] . '</td>' . '<td>' . $row['guestPhoneNo'] . '</td>' . '<td>' . $row['guestAddress'] . '</td> <td><a href="updatef-guest.php?guestID='.$guestID.'"><span class="material-symbols-rounded">edit</span></td> <td><a href="guests-delete.php?guestID='.$guestID.'"><span class="material-symbols-rounded">delete</span></td> </tr>';
    }
    echo '</tbody>';
    echo '</table>';
